...
pub fn wg_http_inject_packet(packet: &[u8]) {
    debug!("wg_http_inject_packet: injecting {} bytes into virtual stack", packet.len());
    ...
    pub fn flush_outgoing(&self) {
        ...
        if crate::wireguard::wg_is_tunnel_active() {
            ...
            for packet in &packets {
                debug!("WG TCP proxy: sending {} bytes via streaming tunnel", packet.len());
    ...
    fn receiver_loop(proxy: Arc<SharedTcpProxy>) {
        ...
        while proxy.running.load(Ordering::SeqCst) {
            ...
            if crate::wireguard::wg_is_tunnel_active() {
                ...
                if retransmitted > 0 {
                    debug!("WG TCP proxy: retransmitted {} segments", retransmitted);
                }
            ...
            match proxy.endpoint_socket.recv(&mut recv_buf) {
                        ...
                        || e.kind() == io::ErrorKind::Interrupted =>
                {
                    ...
                    let retransmitted = proxy.virtual_stack.check_retransmissions();
                    if retransmitted > 0 {
                        debug!("WG TCP proxy rx: retransmitted {} segments", retransmitted);
                    }
                    proxy.flush_outgoing();
                }
                Err(e) => {
